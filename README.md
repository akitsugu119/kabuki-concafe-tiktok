...
- パスコードはサーバー環境変数 `ADMIN_SECRET` で設定する（**リポジトリには書かない**。ローカルは `.env.local`、本番は Render の環境変数）
- `ADMIN_SECRET` 未設定時は管理操作をすべて拒否する（既知の初期値なし）
- ⚠️ 簡易認証のため、将来的にはセッション方式のサーバー認証への置き換えを推奨
